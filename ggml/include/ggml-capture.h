...
struct ggml_capture_q8_data {
    ...
    uint64_t matmul_id;      // sequence number identifying which matmul this belongs to
};
...
// Increment the matmul sequence number. Call once at the start of each
// mul_mat operation (before any dot products fire). Returns the new ID.
uint64_t ggml_capture_next_matmul(void);

// Get the current matmul sequence number.
uint64_t ggml_capture_current_matmul(void);
